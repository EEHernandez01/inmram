...
type SystemUser = { id: string; neonAuthUserId: string; rol: Role; activo: boolean; perfil: { nombreCompleto: string } | null };
...
export function AdminUserForm({ users = [] }: { users?: SystemUser[] }) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      try {
        await save({ neonAuthUserId: created.user.id, rol: newRole, nombreCompleto });
      } catch (saveError) {
  ...
  async function update(user: SystemUser, form: HTMLFormElement) {
    ...
    try {
      ...
      await save({ id: user.id, rol, activo: data.get("activo") === "on" }, "PATCH");
      window.location.reload();
