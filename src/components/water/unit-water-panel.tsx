...
type WaterMeter = {
  id: string;
  cuotaFija: { toString(): string };
  tarifaPorMetroCubico: { toString(): string };
...
export function UnitWaterPanel({
  ...
  return (
    <section className="mt-8 rounded-3xl bg-bg p-6 shadow-[7px_7px_15px_#c6cdd6,-7px_-7px_15px_#fff]">
      ...
      {meter ? <p className="mt-4 text-sm text-ink-secondary">Cuota fija de referencia: <span className="font-semibold text-ink">{formatCurrency(meter.cuotaFija)}</span> · Tarifa por m³: <span className="font-semibold text-ink">{formatCurrency(meter.tarifaPorMetroCubico, 4)}</span></p> : null}

      {!meter ? <div className="mt-5 rounded-2xl border border-dashed border-brand/25 p-4"><p className="text-sm text-ink-secondary">Esta unidad aún no tiene un medidor configurado.</p>{canWrite ? <form action="/api/agua/medidores" className="mt-4 grid gap-4 sm:grid-cols-3" method="post"><input name="unidadId" type="hidden" value={unitId} /><input name="returnTo" type="hidden" value={returnTo} /><label className="text-xs font-medium text-ink-secondary">Cuota fija<MoneyInput className="mt-2" defaultValue="0" name="cuotaFija" required /></label><label className="text-xs font-medium text-ink-secondary">Tarifa por m³<MoneyInput className="mt-2" decimalPlaces={4} defaultValue="0" name="tarifaPorMetroCubico" required /></label><div className="flex items-end"><button className="w-full rounded-xl bg-brand px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand-hover" type="submit">Configurar medidor</button></div></form> : null}</div> : <>
