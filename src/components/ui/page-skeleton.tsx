...
export function PageSkeleton({ variant = "portfolio" }: { variant?: SkeletonVariant }) {
  if (variant === "dashboard") return <div aria-busy="true" role="status"><Header action /><Metrics count={3} /><section className="mt-10"><div className="mb-4 flex justify-between"><SkeletonBlock className="h-6 w-36" /><SkeletonBlock className="h-4 w-36" /></div><div className="grid gap-5 sm:grid-cols-3">{Array.from({ length: 3 }, (_, index) => <div className="rounded-2xl bg-bg p-6 shadow-[7px_7px_15px_#c6cdd6,-7px_-7px_15px_#fff]" key={index}><SkeletonBlock className="h-7 w-20 rounded-xl" /><SkeletonBlock className="mt-5 h-5 w-3/5" /><SkeletonBlock className="mt-3 h-4 w-full" /><SkeletonBlock className="mt-2 h-4 w-4/5" /></div>)}</div></section></div>;
