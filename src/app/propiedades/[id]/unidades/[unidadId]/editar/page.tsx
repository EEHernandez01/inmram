...
import { updateUnitAction } from "@/app/_actions/foundation";
...
export default async function EditUnitPage({ params }: { params: Promise<{ id: string; unidadId: string }> }) {
  ...
  return (
    <>
      ...
      <section className="mt-7 rounded-card border border-border bg-surface p-5">
        <UnitForm action={updateUnitAction.bind(null, unidadId)} defaults={{ identificador: unit.identificador, tipo: unit.tipo, metrosCuadrados: unit.metrosCuadrados.toString(), descripcion: unit.descripcion, piso: unit.piso }} propertyId={id} submitLabel="Guardar cambios" />
