...
export default async function PropertiesPage({ searchParams }: { searchParams: Promise<{ estado?: string }> }) {
  const query = await searchParams;
  const archived = query.estado === "archivadas";
  const [{ user }, properties] = await Promise.all([getSystemUser(), listarPropiedades({ archivadas: archived })]);
  ...
  return <>
    ...
    <section className="mt-7">
      <nav aria-label="Estado de propiedades" className="mb-5 flex flex-wrap gap-2"><Link className={`rounded-pill px-3 py-1.5 text-sm font-semibold transition-colors ${!archived ? "bg-brand text-white" : "bg-bg text-ink-secondary hover:bg-brand-soft hover:text-brand"}`} href="/propiedades">Activas</Link><Link className={`rounded-pill px-3 py-1.5 text-sm font-semibold transition-colors ${archived ? "bg-brand text-white" : "bg-bg text-ink-secondary hover:bg-brand-soft hover:text-brand"}`} href="/propiedades?estado=archivadas">Archivadas</Link></nav>
      {properties.length === 0 ? <EmptyState action={archived ? undefined : newLink} description={archived ? "No hay propiedades archivadas." : "Agrega el primer inmueble para comenzar a registrar sus unidades y contratos."} title={archived ? "Sin propiedades archivadas" : "Tu portafolio está vacío"} /> : <>
