  ...
  return (
    <>
      ...
      <section className="mt-7 rounded-card border border-border bg-surface p-5">
        ...
        <PropertyForm action={`/api/propiedades/${id}`} defaults={{ direccion: property.direccion, googlePlaceId: property.googlePlaceId ?? undefined, latitud: property.latitud?.toString(), longitud: property.longitud?.toString(), valorCatastral: property.valorCatastral.toString(), valorComercialTotal: property.valorComercialTotal.toString(), predialAnual: property.predialAnual.toString(), mantenimientoAnual: property.mantenimientoAnual.toString(), propietarioId: `propietario:${property.propietarioId}` }} existingPhotos={property.archivos.map(({ id: photoId, nombre, url }) => ({ id: photoId, nombre, url }))} owners={owners} placesEnabled={Boolean(process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY)} submitLabel="Guardar cambios" />
