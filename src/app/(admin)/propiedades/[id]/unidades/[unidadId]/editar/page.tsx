  ...
  return (
    <>
      ...
      <section className="mt-7 rounded-card border border-border bg-surface p-5">
        ...
        <UnitForm defaults={{ propietarioId: unit.propietarioId, identificador: unit.identificador, tipo: unit.tipo, metrosCuadrados: unit.metrosCuadrados.toString(), descripcion: unit.descripcion, piso: unit.piso, recamaras: unit.recamaras, banosCompletos: unit.banosCompletos, mediosBanos: unit.mediosBanos, amenidades: Array.isArray(unit.amenidades) ? unit.amenidades.map(String) : [], amenityDetails: unitAmenityDetailsFrom(unit.atributos) }} owners={owners} propertyId={id} submitLabel="Guardar cambios" unitId={unidadId} />
