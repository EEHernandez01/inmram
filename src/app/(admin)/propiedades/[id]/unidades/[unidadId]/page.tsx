...
import { unitAmenityDetailsFrom } from "@/lib/unit-amenities";
...
export default async function UnitDetailPage({ params }: { params: Promise<{ id: string; unidadId: string }> }) {
  ...
  const amenityDetails = unitAmenityDetailsFrom(unit.atributos);
  const amenities = Array.isArray(unit.amenidades) ? unit.amenidades.filter((amenity): amenity is string => typeof amenity === "string") : [];
  const formatAmenity = (amenity: string) => {
    if (amenity === "Roof garden" && amenityDetails.roofGardenMetrosCuadrados) return `Roof garden · ${amenityDetails.roofGardenMetrosCuadrados} m²`;
    if (amenity === "Balcón" && amenityDetails.balconMetrosCuadrados) return `Balcón · ${amenityDetails.balconMetrosCuadrados} m²`;
    if (amenity === "Estacionamiento" && amenityDetails.cajonesEstacionamiento) return `Estacionamiento · ${amenityDetails.cajonesEstacionamiento} ${amenityDetails.cajonesEstacionamiento === 1 ? "cajón" : "cajones"}`;
    return amenity;
  };
